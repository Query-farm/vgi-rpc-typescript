...
- **conformance** job: installs `vgi-rpc[http,cli,external,conformance]` from a checkout of `Query-farm/vgi-rpc-python` **main**, unpinned — not from PyPI, so there is no version floor to raise when the reference adds conformance cases; they arrive the day they land (the `conformance` extra carries jsonschema, required by the access-log validator), runs conformance + client tests with all transports, and validates the emitted access log via `vgi-rpc-test --access-log ... --require-request-data` (unfiltered, so zero-parameter methods like `void_noop` stay in scope) — twice: once against the pipe worker and once against the HTTP worker over `--url`, each followed by `conformance/check_access_log_streams.py`. The HTTP pass is the one that matters for streams, since HTTP is the only transport where a stream's turns are separate requests that have to be chained, and it went entirely unvalidated until 0.37.
