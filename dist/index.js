    ...
    try {
      ...
      if (parts.length >= 2) {
        ...
        if (claims.exp != null) {
          ...
          return { token: token2, maxAge, refreshToken };
        }
      }
    } catch {}
    return { token: token2, maxAge: AUTH_COOKIE_DEFAULT_MAX_AGE, refreshToken };
  }
...
var USER_INFO_STYLE = `#vgi-user-info {
...
function buildUserInfoScript(cookieName, logoutUrl) {
  return `(function() {
...
function buildUserInfoHtml(prefix) {
  const logoutUrl = `${prefix}/_oauth/logout`;
  return `<style>${USER_INFO_STYLE}</style>
...
` + `  Direction: ${direction}`);
  ...
  let landingHtml = enableLandingPage ? buildLandingPage(displayName, serverId, enableDescribePage ? `${prefix}/describe` : null, repoUrl) : null;
