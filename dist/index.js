...
` + `  Direction: ${direction}`);
  ...
  return async function handler(request) {
    ...
    const exemptFromMaxBytes = path === healthPath || path === `${prefix}/${UPLOAD_URL_METHOD2}/init` || path === `${prefix}/__capabilities__`;
    ...
    if (path === `${prefix}/${UPLOAD_URL_METHOD2}/init`) {
      if (!uploadUrlProvider) {
        return new Response("Not Found", { status: 404 });
      }
      try {
        const { schema: reqSchema, batch: reqBatch } = await readRequestFromBody(body);
        const parsed = parseRequest(reqSchema, reqBatch);
        if (parsed.methodName !== UPLOAD_URL_METHOD2) {
          throw new HttpRpcError(`Method name in request '${parsed.methodName}' does not match URL '${UPLOAD_URL_METHOD2}'`, 400);
        }
...
//# debugId=B72F8C2157A0CBD664756E2164756E21
