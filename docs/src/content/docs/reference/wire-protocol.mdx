...
- Batch metadata carries the routing key, the method name and the protocol version:
  ...
  - `vgi_rpc.protocol` — **required**: which of the server's hosted protocols the method belongs to. Dispatch resolves the pair `(protocol, method)`, so method names may collide across protocols. There is no single-protocol exemption: an intermediary that rebuilds a request and drops the field must be told, not landed silently on whichever protocol happened to be first. On HTTP this field is *canonical* and the `{prefix}/{protocol}/{method}` path segment is its required projection; see [HTTP transport](/guides/http-transport/).
