...
HTTP body cap. Hard for unary and stream-exchange responses (overshoot surfaces as `200` + an `X-VGI-RPC-Error` EXCEPTION batch). For producer streams it is a worker-visible soft budget for one invocation; producer dispatch remains lock-step and never invokes the handler more than once per request. Externalized payloads do not count toward this. Advertised via `VGI-Max-Response-Bytes`.
...
For compatibility this value takes precedence over `maxResponseBytes` as the worker-visible producer budget. It does not change the one-request/one-invocation stream shape.
