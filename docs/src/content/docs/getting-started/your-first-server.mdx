...
```typescript
import { Protocol, VgiRpcServer, float } from "@query-farm/vgi-rpc";
...
The `params` and `result` objects use [schema shorthand](/guides/schema-shorthand/) — `float` expands to `Float64` Arrow fields automatically.
...
Setting `enableDescribe: true` hosts `vgi_rpc.Reflection.v1`, the co-hosted protocol a client
introspects through. It is the default.
