...
- **`doc`** — documentation string kept with the method definition.
...
`VgiRpcServer` wraps the protocol and handles the stdin/stdout request loop. Setting `enableDescribe: true` hosts `vgi_rpc.Reflection.v1`, the introspection protocol (the default).
