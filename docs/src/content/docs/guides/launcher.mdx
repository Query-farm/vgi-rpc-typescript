...
- **One server process per socket.** `serveUnix` binds an AF_UNIX socket, accepts connections one at a time (sequential listen, like Python's `serve_unix`), and dispatches each connection through the standard unary / stream machinery.
