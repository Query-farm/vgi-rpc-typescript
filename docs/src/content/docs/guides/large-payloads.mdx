...
Producer dispatch remains lock-step: one HTTP request invokes the producer exactly once. If an invocation stays within its limit and leaves the stream unfinished, the response carries a zero-row continuation-token batch and the client resumes by calling `/{method}/exchange` with that token.
...
:::note
`maxStreamResponseBytes` is the deprecated, producer-only predecessor of `maxResponseBytes`. It is now a compatibility alias for the application hard cap when `maxResponseBytes` is absent. Prefer `maxResponseBytes` for new code.
:::
