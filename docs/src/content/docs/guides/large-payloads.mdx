...
:::note
`maxStreamResponseBytes` is the deprecated, producer-only predecessor of `maxResponseBytes`. If both are set, the producer loop prefers `maxStreamResponseBytes` for its soft budget; otherwise it falls through to `maxResponseBytes`. Prefer `maxResponseBytes` for new code.
:::
