...
Connect to an HTTP server programmatically:
...
// Producer stream
const stream = await client.stream("count", { limit: 5 });
...
See [Client Transports](/guides/client-transports/) for details on all transport options.
