...
Pass an `authenticate` callback to protect your endpoints. vgi-rpc ships with factories for bearer tokens, JWT, mTLS, and XFCC. See the [Authentication guide](/guides/authentication/) for full details.

Quick example with bearer tokens:

```typescript
import { createHttpHandler, bearerAuthenticateStatic, AuthContext } from "vgi-rpc";
...
```typescript
import { chainAuthenticate, mtlsAuthenticateSubject, bearerAuthenticateStatic } from "vgi-rpc";
