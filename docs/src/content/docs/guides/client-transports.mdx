...
The vgi-rpc client library provides HTTP, subprocess, pipe, and raw-TCP connect functions that all return the same `RpcClient` interface. Pick the transport that fits your deployment and use `call()`, `stream()`, `describe()`, and `close()` identically across all of them.
