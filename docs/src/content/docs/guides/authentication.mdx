...
Built on WebCrypto rather than `node:crypto`, so it works on workerd and in browsers as well as
Node and Bun.

```ts
import { parseProofSecrets, requireProxyProof } from "@query-farm/vgi-rpc/http";

const authenticate = requireProxyProof(
  {
    mode: "require",
    originId: "worker-a",
    secrets: parseProofSecrets(`prod-use1:${hexSecret}`),
  },
  existingAuthenticate, // optional — omit for proof-only
);

const handler = createHttpHandler(protocol, { authenticate, proxyProofRequired: true });
```

:::caution
This composes as an **AND**, not an alternative. Do **not** pass the gate to `chainAuthenticate` —
chaining is first-return-wins, so any later credential would bypass it. Pass your existing
authenticator as the second argument instead.
:::

| option | type | default | description |
|---|---|---|---|
| `mode` | `"off" \| "allow" \| "require"` | — | `allow` verifies and records without denying; the rollout lever |
| `originId` | `string` | — | This worker's identifier; folded into every MAC but never transmitted |
| `secrets` | `Record<string, ProofSecret>` | — | Key id → secret and label |
| `skewSeconds` | `number` | `30` | Half-width of the timestamp acceptance window |
| `replayCapacity` | `number` | `100000` | Hard bound on the nonce cache |
| `replayCache` | `boolean` | `true` | Whether replayed nonces are rejected |

`kid` doubles as the calling proxy's label, so `ctx.claims.vgi_proxy_proof.proxy` records *which*
proxy served each request. The label comes from the secret that verified, never from the
transmitted field — a caller can claim any `kid` but cannot produce its MAC.

`OPTIONS`, `/.well-known/` and `{prefix}/health` stay reachable without a proof in every mode, so
load-balancer probes keep working.

### Advertising the posture

`proxyProofRequired: true` makes the handler emit `VGI-Proxy-Proof-Required: true` on every
response, including `GET {prefix}/health`, and adds it to `Access-Control-Expose-Headers`. It is
advertisement only — it enforces nothing, and the gate above is what denies.

Set it only with `mode: "require"`. `allow` never denies, so advertising there would tell a proxy
the hop is protected when it is not. The declaration is manual because the gate reaches the handler
as an opaque `AuthenticateFn`, which the handler cannot introspect for its mode; without the header
a proxy has no way to distinguish an enforcing worker from one that silently ignores the header —
exactly the misconfiguration that turns the feature into a no-op.

The normative cross-language contract is `docs/proxy-proof-spec.md` in the vgi-rpc repository.
...
For interactive, browser-based sign-in, vgi-rpc can serve RFC 9728 OAuth Protected Resource Metadata, run the PKCE authorization-code redirect flow, handle the `/_oauth/*` routes, and emit `WWW-Authenticate` challenges. See the [OAuth guide](/guides/oauth/) for the full setup, including the device-code flow and how the cookie session described above is issued.
